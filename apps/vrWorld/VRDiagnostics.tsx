...
export default function VRDiagnostics({ characters, api }: { characters: CharacterProfile[]; api: APIConfig }) {
    ...
    return <section className="space-y-3 p-5" style={{ background: F.surface, color: F.textPrimary, borderRadius: R.bigCard, boxShadow: S.raisedSoft }}>
        ...
        <button disabled={busy} onClick={() => void collect()} className="min-h-11 w-full px-4 text-sm" style={{ background: F.surface, borderRadius: R.button, boxShadow: S.raisedSoft }}>{busy ? '读取中…' : '读取并复制诊断'}</button>
