...
const libraryTokens = { '--vrl-text': F.textPrimary, '--vrl-sub': F.textSecondary, '--vrl-accent': HUE.violet.ink, '--vrl-tint': HUE.violet.tint, '--vrl-bg': F.appBg, '--vrl-sunken': F.surfaceSunken, '--vrl-surface': F.surface, '--vrl-line': F.borderSoft, '--vrl-shadow': S.raisedSoft, '--vrl-inset': S.sunken, '--vrl-button': `${R.button}px`, '--vrl-card': `${R.bigCard}px`, '--vrl-sheet': `${R.sheet}px`, '--vrl-danger': STATUS.danger.ink } as React.CSSProperties;
...
export function LibraryView({ novels, categories, characters, onOpen, onAdd, onDelete, onEdit, onPreference }: {
    ...
    return <section style={libraryTokens} className="vr-library" aria-label="彼方书库">
