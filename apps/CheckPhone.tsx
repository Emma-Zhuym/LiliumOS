...
const CheckPhone: React.FC = () => {
    ...
    useEffect(() => {
        ...
        window.addEventListener(CHECK_PHONE_API_CHANGED_EVENT, sync);
        return () => window.removeEventListener(CHECK_PHONE_API_CHANGED_EVENT, sync);
    }, []);
        ...
        return (
            <div className="absolute inset-0 flex flex-col overflow-hidden" style={{ background: '#f4f1ea' }}>
                ...
                <div className="h-14 flex items-center justify-between px-4 shrink-0">
                    ...
                    <button
                        onClick={() => { setPhoneApiTestResult(null); setShowApiSettings(true); }}
                        aria-label="查手机 API 设置"
                        className="relative w-9 h-9 rounded-full flex items-center justify-center active:scale-90 transition"
                        style={{ color: CF.text, background: '#e8e2d6' }}
                    >
                        <GearSix size={17} weight={phoneApiFollowsDefault ? 'regular' : 'fill'} />
                        {!phoneApiFollowsDefault && <span className="absolute right-1.5 bottom-1.5 h-1.5 w-1.5 rounded-full bg-violet-500" />}
                    </button>
                </div>
                {(() => {
                    const PER_PAGE = 6;
                ...
                <Modal isOpen={showApiSettings} title="查手机 · API 设置" onClose={() => setShowApiSettings(false)}>
                    <div className="space-y-3">
                        <p className="text-[11px] leading-relaxed text-slate-500">
                            查手机里的内容生成、联系人对话、智能体和人格模拟都会走这里。单独选择不会影响普通聊天；不设置则跟随聊天默认。
                        </p>
                        <div className="rounded-2xl bg-slate-50 border border-slate-200 p-3.5">
                            ...
                            <button
                                onClick={() => void testPhoneApi()}
                                disabled={testingPhoneApi}
                        ...
                        {apiPresets.length === 0 ? (
                            ...
                            return (
                                <button
                                    key={preset.id}
                                    onClick={() => choosePhoneApi(preset.config)}
