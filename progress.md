...
- 已做状态时钟、MCP/网络请求核心修复，以及文字语音收藏的存储/快照/备份核心；收藏 UI 和回复操作等其余功能未接入，不能视为已经可用。
- 异格方案确定为剧情内独立入口，自由选择人格/世界 + 本地随机，不依赖活动室货币/抽取；尚未实现。
- GitHub 新备份核心与发布回执恢复修复保存为待接入补丁，须与设置界面一起应用。
- `APP_CONVENTIONS.md §0` 在本机缺失，现有设计规范替代方案已询问、待回复。完整授权、验证和续接材料见 [本轮验收记录](docs/upstream-sync-2026-09-12.md)。
