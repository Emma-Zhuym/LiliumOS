...
- main 与 origin/main 锁定 `b61078f6`；在 `merge-upstream-2026-09-12` 分阶段接入用户所选范围，尚未完成整批同步。
- 已做状态时钟、MCP/网络请求核心修复，以及文字语音收藏的存储/快照/备份和聊天/见面/通话入口；收藏不要求 TTS，不能视为整批同步完成。
- 已接入私聊/群聊的底部生成按钮、回车发送、发送后自动回复、表情联想，默认沿用原行为；19 项专项 + 全量 4167 项通过（5 跳过），EM 83/83 与构建通过。本地浏览器验证私聊设置、两行草稿和按钮切换；手机和群聊界面尚未实测。
- 语音 UI 后全量 4181 项通过（5 跳过）、35 项专项、EM 83/83、前端和 Worker 构建通过，类型仍为原 47 条。本地浏览器验证装饰语音收藏、搜索、说话者/换行与独立取消；见面/通话完整页面及手机体验未实测。
- 异格方案确定为剧情内独立入口，自由选择人格/世界 + 本地随机，不依赖活动室货币/抽取；尚未实现。
- DB 71 兼容、写入提交、SDK/旁路清理、统一 firePack 原文范围和后台暂停入口已接入；全量 4277 项通过（5 跳过）、EM 83/83、前端与 6 Worker 构建通过；类型保持已知 50 条。副本升级与新表恢复回滚通过，未操作线上 Worker。
- 前台 3 秒收件巡查、后台首次到达状态保留、双通道诊断与表情分类前缀已接入；全量 4317 项通过（5 跳过），EM 83/83、前端与 6 Worker 构建通过。本地浏览器确认 SW 1.18.0、两路回信及完整记录复制；真实推送、手机与跨页面整段处理去重未验收。
- 查手机异步合并/统一原文范围、日程分寸提示、独立 API 与记忆配置恢复、API 预设即时切换和独立表单已接入；全量 4356 项通过（5 跳过）、EM 83/83、前端与 6 Worker 构建通过，类型保持已知 50 条。390×844 浏览器确认预设编辑/取消/滚动/固定保存钮，未请求真实模型或更新后台任务。
- GitHub 32 MB 分片、完成清单、发布回执丢失保护、明确中转选择和缺片禁止恢复已接入；完整 4206 项通过（5 跳过）、EM 83/83、前端和 6 Worker 构建通过。类型为原 47 条 + 3 处上游测试纯类型诊断；本地浏览器确认默认直连、凭据说明和 Clay 底部面板，未连接或上传真实备份。
- 已完整读过 Emma 新提供的根目录 `APP_CONVENTIONS.md` v0.2；其无 §0，顶栏依仓库更具体的 16px/600 规则，其余遵守公约。完整授权、验证和续接材料见 [本轮验收记录](docs/upstream-sync-2026-09-12.md)。
