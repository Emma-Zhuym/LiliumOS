...
- 开发分支 `codex/finance-spending-scope`：手动标记代收代付、一次性支出、其他排除或始终保留；金额分位数 P50–P100；并列完整、手动排除、再排大额三组比较。默认手动口径/P95，原始流水与余额保留。
- 页面、角色支出汇总共用计算；标记保留到 SimpleFIN 更新与完整备份/恢复。代充收款和付款分别标记，收入不参与分位数。
- 56 项相关测试、生产构建通过；390×844 本地浏览器已验证标记、收入排除、P100、非法值、刷新持久化和单笔编辑。详细方案与验收见 `docs/finance-spending-analysis.md`。
