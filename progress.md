...
## 已验收发布：2026-09-16 花销分析口径

- 开发分支 `codex/finance-spending-scope`：手动标记代收代付、一次性支出、其他排除或始终保留；默认 IQR 上界（P75 + 1.5 × 四分位距），可切换金额分位数 P50–P100；并列完整、手动排除、再排大额三组比较。默认手动口径/IQR，原始流水与余额保留。
- 页面、角色支出汇总共用计算；标记保留到 SimpleFIN 更新与完整备份/恢复。代充收款和付款分别标记，收入不参与分位数。
- 分析页逐笔管理列表已按用户要求移除，标记保留在交易编辑；IQR 设置改为紧凑折叠行。界面更新为「全部/个人/日常」横向切换 + 日期柱状图，设置说明与分类分布折叠；增加多日合成样本与可切换中位数/均值参考线；67 项相关测试、生产构建通过；默认 IQR、旧设置升级和方法切换已验证；390×844 本地浏览器已验证标记、收入排除、P100、非法值、刷新持久化和单笔编辑。详细方案与验收见 `docs/finance-spending-analysis.md`。
- Emma 已完成界面验收并明确批准推送 main；线上部署另行核验。真实账本、真 iPhone 和真实角色回复仍未由自动检查覆盖。全仓类型检查有 50 条既存问题，本次修改文件无报错；EM 检查 82/83，缺的是基线已有的聊天搜索文案锚点，本次未改聊天。
...
- 同步同时检查本地历史，修复超出抓取窗口的旧 hold；重放仍排除，旧记录不再计入新交易提醒。16 项新增合成数据测试与 27 项既有 Finance/SimpleFIN 测试、生产构建、diff 检查通过；全仓类型检查仍有既存错误，本次 SimpleFIN 文件无报错。尚未在真实账本同步验收，本次单独接入 main，不包含厨房分支改动；正式部署及真实账本效果待核验。
