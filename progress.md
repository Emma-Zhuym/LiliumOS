...
- 界面更新为「全部/个人/日常」横向切换 + 日期柱状图，设置说明与分类分布折叠；增加多日合成样本与可切换中位数/均值参考线；67 项相关测试、生产构建通过；默认 IQR、旧设置升级和方法切换已验证；390×844 本地浏览器已验证标记、收入排除、P100、非法值、刷新持久化和单笔编辑。详细方案与验收见 `docs/finance-spending-analysis.md`。
