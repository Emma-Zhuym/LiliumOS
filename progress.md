...
- 修复主动消息 Worker 的更新源串包：一键部署、自更新、教程和手动代码入口统一指向 LiliumOS `worker/amsg`；自更新后会等待并复核 Worker 自报版本，旧上游包不再被显示成成功。已经装过错误更新源的旧 Worker 需在 Cloudflare 手动覆盖一次当前 bundle，之后可继续在设置页更新。
...
- Worker 更新源修复（2026-08-18）：更新源、自更新、即时聊天与 Worker fire 相关 9 个测试文件 595 tests passed；AMSG bundle 已重建，生产构建与 EM 79/79 哨兵通过。
