...
## 待实机验收：2026-09-15 SimpleFIN Instacart 调价预授权

- `simplefinInstacart.ts` 针对已确认的短名称 `INSTACART 159` / 带电话的正式扣款格式，按同账户、同币种、同日分组；仅一条短名称记录且至少一条非 pending 正式支出时排除旧记录，允许多条正式扣款并保留各自金额、分类和备注。多条短名称候选或跨日不猜测匹配，不改变来源 pending 状态，不删除原始记录。
- 同步同时检查本地历史，修复超出抓取窗口的旧 hold；重放仍排除，旧记录不再计入新交易提醒。16 项新增合成数据测试与 27 项既有 Finance/SimpleFIN 测试、生产构建、diff 检查通过；全仓类型检查仍有既存错误，本次 SimpleFIN 文件无报错。尚未在真实账本同步验收，未部署到 main。

## 开发中：2026-09-13 Lilian Kitchen 第一块

- `codex/kitchen-inventory-core` 已加入可从模拟手机桌面打开的「小厨房」App，使用 Clay 库存界面，并开始接入第一版 3D 冰箱；菜谱、Health 和角色工具尚未实现。
- 本地库存按食品与购买批次分开保存；整件计数与可分食包装使用同一事件账本。可分食包装支持开封余量、自由填写分数/百分比或 ml、L、g、kg、oz、fl oz、lb，余量核对、一键用完和连余量一起撤销，重复 operationId 不会重复扣减。包装规格仍保存为原文字段，换算层只把本次输入转换为统一比例，不迁移旧库存。
- 厨房食品、批次和事件已接入文字/完整备份恢复。自动测试覆盖 6 个鸡蛋 → 吃掉 1 个 → 撤销、重复提交、超量扣除、丢弃/核对分流及备份往返。
- 数量换算与库存账本定向测试 22 项通过，生产构建通过；390×844 浏览器除原有鸡蛋流程外，已实测 1 L 牛奶消耗 250 ml 后显示约剩 750 ml、核对为 700 ml、撤销恢复 750 ml，并确认把重量 oz 与容量 ml 混填时会提示改用 fl oz。正式 iPhone PWA 尚未验收；EM 哨兵检查另有当前分支既存的“聊天页搜索入口”1 项缺失，与本轮厨房文件无关。
- 2026-09-13 审查修复：按 Emma 要求，“这条库存已用完”清零整批而非扣一件，并保留 CONSUME 事件、重复提交保护与完整撤销；余量填 0 同样清零，丢弃半件不再产生负数。百分号缺数字拒绝入账；编辑回填不截断数值，近似无变化保存保留原余量、跳过撤销并从可见历史隐藏零变化记录。
- 库存改为一张紧凑列表，默认折叠、一次展开一条；增加名称搜索与位置联合筛选，消耗后保持原行位置。测试使用 20 条隔离食材覆盖筛选、展开、错误输入及清零/撤销；真实 390px 浏览器验证搜索、空状态、百分号拦截与清零恢复。既有用户库存保留，试用牛奶清零后已撤销恢复。
- 本轮最终验证：3 个厨房测试文件共 40 项通过，生产构建通过；全仓类型检查仍有既有错误，厨房文件无报错。EM 哨兵仍为 82/83，缺失项与本轮无关。正式 iPhone PWA 待用户体验；本轮交接未写入 Engram。
- Emma 已把下一块优先级改为可视化：普通单开门冰箱与旁边有门的 pantry，先完成冰箱交互小版，再扩展 pantry。
- 2026-09-13 食材详情与编辑已完成：列表展开区可进入独立详情页，修改名称、收纳单位、包装规格、位置、购买/到期日期。可分食包装始终保存剩余比例，改规格只重新换算展示值，不生成扣减/核对账本事件；改食材名会同步同名批次。详情页只显示当前批次的历史，含已撤销记录。厨房定向测试现为 42 项通过，生产构建和本地手机预览通过；正式 iPhone PWA 待用户体验。

- 2026-09-13 第一版 3D 冰箱已接入：Kenney 素材整理为带独立门轴和三层隔板的 GLB，冷藏库存每组最多 9 条自动摆放，可点模型进入详情；详情增加复用原账本的整条清零入口。49 项厨房测试与生产构建通过；390×844 浏览器确认带贴图画面、开门及鸡蛋模型点击详情。未在用户库存试扣；清零/撤销以隔离测试验证。全仓既存类型错误和 EM 82/83 未变，正式手机待验收，Engram 交接未写入。下一步 pantry，详见 `docs/kitchen-3d.md`。

- 2026-09-13 Emma 反馈小冰箱过于方正，已重做原创圆角上下双门模型：上冷冻下冷藏、双透明保鲜抽屉、玻璃隔板、内胆、门封、金属把手；烤漆与金属微纹理嵌入 GLB。抽屉支持拉出/推回，冷冻库存可切换查看；库存数据不变。50 项厨房测试、构建及浏览器检查结果见 `docs/kitchen-3d.md`，正式手机和 Blender 导入待验收。

- 2026-09-13 冰箱交互更新：移除冷藏/冷冻标签，直接点两扇门各自开关；点食材可选冷藏层板、门内上层或门内下层，模型与编号随门移动。摆放位置本地保存并随批次备份，数量/余量不变。53 项定向测试、构建通过；浏览器实测独立开门、测试牛奶移动及重新进入后的恢复，已放回原层板。正式手机待体验，Engram 未写入。

- 2026-09-13 鸡蛋改用原创两排六格蛋盒，按个记录时随剩余数量显示最多 12 个鸡蛋；门架增加中层并加深，蛋盒占整层宽度，其他食材顺延翻组。55 项厨房测试与构建通过，浏览器确认现有 5 个鸡蛋及三层门架，未改动用户库存或摆放。正式手机待体验，Engram 未写入。

- 2026-09-13 按 Emma 反馈取消冰箱翻组：所有食材在同一个场景中显示，每层按总宽度调整模型大小，门架指定位置不变。53 项厨房测试和构建通过，浏览器确认全部 4 条现有库存同时出现；未修改库存。此前翻组方案废弃。

- 2026-09-14 冰箱按珍珠白参考做首轮细化：白色烤漆、银色把手、门封层次、透明架边缘与接触阴影；冷冻门增加两层可摆放门架，冷冻层板为上下两层。共享几何后冰箱约 1.8 MB。56 项厨房测试、生产构建及 390px 隔离浏览器移动/独立开门检查通过，真实库存未写入测试数据；手机性能与进一步食材美术待体验，Engram 未写入。

- 2026-09-14 修正透明抽屉在白色内胆上难以看清的问题：正面淡灰蓝、侧壁与底边加深，补塑料接缝和顶沿亮线，保留透明空心结构及开合。冰箱约 1.9 MB；10 项模型/摆放测试、生产构建与 390px/正常宽度浏览器外观检查通过，库存不变，正式手机与 Blender 效果待验收。

- 2026-09-14 加强冰箱光影：定向主光与侧后补光拉开明暗，层板下方/内壁转角/抽屉底部补柔边暗部；内灯随各门打开亮起。仍只有一张实时阴影图，按需绘制。14 项相关检查与构建通过，厨房类型无报错；390px/正常宽度浏览器确认外观及关门收抽屉，正式手机性能待验收。

- 2026-09-14 按 Emma 选择加入浅蓝墙＋浅木地板背景、白色踢脚线、淡木纹/错缝与墙地投影；pantry 后续放冰箱右侧，本轮仍只展示冰箱。背景不参与点击，库存不变；14 项相关测试、生产构建通过，手机宽度检查配色与点门/选食材正常，厨房类型无报错，实机性能待验收。

- 2026-09-14 按连续手机体验反馈调整构图：冰箱正面对齐、门展开 112°，画布随宽度收紧以缩掉前方空地；冷冻门加高到约三分之一，冷藏尺寸不变。编号碰撞时自动让开 44px 点击区。原 pantry 右侧并排改为独立一页（柜子尚未实现）。18 项相关测试、构建通过，厨房类型无报错；390×844 浏览器验证外观，库存不变。

## 已实现待体验：2026-09-12 查手机短信与固定联系人


- 已确认旧行为：短信页只读且停更；通讯录扫描每次生成 4–6 人并改写已有关系/好感/最近互动；偷看无另起话题分支。
- 联系人扫描现只新增、不覆盖已有资料。首次建议 4–6 人，已有通讯录时建议 0–2 人，允许没有新人。
- 短信页恢复刷新：每次从现有好友中选最多 3 人开启新话题；对话内“继续偷看”延续当前话题。真实角色继续双向同步，NPC 沿用单次生成。
- 完整旧原文保留；记录新增 topicStart 标记，续写只取当前话题，旧话题尾部用于提示新话题避免重复。删除消息时同步调整边界；完整/文字备份保留该字段。旧版独立归档仍可阅读和绑定。
- 22 项相关测试及前端构建通过。浏览器确认新版短信入口与说明；未调用真实模型，新话题实际内容质量待 Emma 体验。既有 tsc 49 条问题未扩展处理。
- 当前仅验收分支，main 未推进；本次完成摘要尚未写入 Engram。

## 固定交互：2026-09-12

- 按 Emma 要求移除“发送按钮代替生成按钮”开关，私聊/群聊固定使用聚焦发文字、失焦触发回复的交互；旧备份关闭值也统一读取为开启。独立聊天设置页为常驻入口。
- 只复查输入偏好与现有输入交互测试；旧测试已补上真实打字前的聚焦步骤。
