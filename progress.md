...
- 本地库存按食品与购买批次分开保存；整件计数与可分食包装使用同一事件账本。可分食包装支持开封余量、自由填写分数/百分比、余量核对、一键用完和连余量一起撤销，重复 operationId 不会重复扣减。
- 厨房食品、批次和事件已接入文字/完整备份恢复。自动测试覆盖 6 个鸡蛋 → 吃掉 1 个 → 撤销、重复提交、超量扣除、丢弃/核对分流及备份往返。
- 生产构建通过；390×844 浏览器实测添加鸡蛋、扣减、撤销与重新打开后仍为 6 个，并实测一大瓶牛奶消耗 1/3 后显示开封约剩 2/3。正式 iPhone PWA 尚未验收。
