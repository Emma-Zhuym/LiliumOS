...
- 单角色私聊的通用 MCP 调用现在会在本轮角色回复下方留下可展开记录：默认只显示工具名与成功/失败，展开后可查看服务器、发送参数、返回结果和耗时；标准 function calling 与正文兼容兜底两条路径均覆盖。记录只写入消息 metadata，不进入后续聊天正文；常见凭据字段会在落库前打码，过长内容截断。
...
- MCP 调用记录（2026-09-15）：打码/限长/坏数据边界、MCP 客户端、协议核心与工具调用兼容共 4 个测试文件、79 passed；生产构建、`check-em-patches.sh` 83/83 与 `git diff --check` 通过，折叠/展开及 390×844 手机宽度完成浏览器检查。
