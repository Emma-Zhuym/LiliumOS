...
- 3D 场景作为库存页的可选视图，按需加载；外层使用中性凹陷底色，视图切换沿用凹槽与白色选中项。场景画布按 Emma 2026-09-14 选择使用浅蓝墙＋浅木地板、白色踢脚线，后续 pantry 放冰箱右侧；房间材质沿用 Clay 蓝/棕色系，木纹与投影属于 3D 场景，不延伸到界面背景。画布用 token 圆角裁切，背景不参与点击。模型保留素材本身配色，按钮与文字只用 Clay tokens。
