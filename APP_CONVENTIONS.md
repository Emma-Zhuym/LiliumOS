...
- 每页 4 列 × 6 行；软件图标和文件夹占 1×1，方形组件占 2×2，常规横卡占 4×2。所有布局项都能进入编辑模式拖动，位置保存在 theme 中。
- 默认最左页是合并的月历与近期事件；中间的时钟页是启动默认页；右页放日程、音乐、图片和软件。默认位置只是初始排列，用户移动后保留其选择。
- 六行格子按页面宽度计算，2×2 组件保持正方形，4×2 横卡保持固定比例；时钟页与日程页用顶部留白保留原画面的纵向节奏，页面可在较矮屏幕上纵向滚动。
