...
- 第二屏的日程、存钱罐、健康、共栖舱共用同一张卡位。上下滑或点右侧按钮手动换卡，不自动翻页；点卡片主体进入对应软件。切卡手势不能触发进入软件。
- 小组件共用卡片尺寸、色彩 token、翻页导航和点击行为；每张卡独立读取自己的数据。存钱罐只读，按日或按分类展示本月支出，精确排除「每月固定／房租」二级分类。健康只展示周期第几天；共栖舱用 2×2 排列最多四个灯或风扇，图标和名称直接放在卡片底上，只有开关使用左右滑块胶囊，不放亮度调节，操作状态与失败信息留在卡内，并明确演示状态。
