...
- 3D 场景作为库存页的可选视图，按需加载；外层使用中性凹陷底色，视图切换沿用凹槽与白色选中项。场景画布按 Emma 2026-09-14 选择使用浅蓝墙＋浅木地板、白色踢脚线，手机比例体验后改为 pantry 独立一页，不强塞冰箱右侧；房间材质沿用 Clay 蓝/棕色系，木纹与投影属于 3D 场景，不延伸到界面背景。画布用 token 圆角裁切，宽高比 1:1.18、最高 420px；相机正面对齐，脚边只留窄地面，背景不参与点击。模型保留素材本身配色，按钮与文字只用 Clay tokens。
- 全部库存同场展示，不分页；一个模型代表一个批次，编号对应下方名称按钮，44px 点击区域同时提供文字入口。
- 冷藏门三层、冷冻门两层，各区只展示有效摆放位置。食品跟随各自门轴；跨收纳分区后回到层板，数量不变。
- 冰箱不使用分区标签，直接点击上下门独立开合；关门时隐藏对应食材编号。所有冷藏/冷冻食材同时放在一个冰箱中，不分页或翻组。层板按占用宽度均衡排列，门架保留用户指定层；每层根据食材总宽度等比缩小模型，蛋盒按更宽的占位参与排列。四个摆放按钮用两列布局。加载失败保留列表；减少动态效果时，门与抽屉直接到最终位置。各门保留键盘/读屏等价按钮，透明抽屉保留文字开合入口。
