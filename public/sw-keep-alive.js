...
// utils/swTraceStore.ts
var DB_NAME = "ActiveMsgSwTrace";
var STORE = "entries";
var VERSION = 2;
var LIMIT = 300;
var connection = null;
...
async function appendSwTraceEntry(entry) {
...
// utils/inboxDelivery.ts
function mergeInboxDelivery(incoming, existing) {
  if (!existing) return incoming;
  return {
    ...existing,
    ...incoming,
    receivedAt: Number.isFinite(existing.receivedAt) && existing.receivedAt > 0 ? existing.receivedAt : incoming.receivedAt,
    receivedWhileVisible: typeof existing.receivedWhileVisible === "boolean" ? existing.receivedWhileVisible : incoming.receivedWhileVisible,
    metadata: {
      ...existing.metadata,
      ...incoming.metadata,
      ...existing.metadata?.amsgOutboxBackfill === true || incoming.metadata?.amsgOutboxBackfill === true ? { amsgOutboxBackfill: true } : {}
    }
  };
}
...
function traceSw(event, payload, extra = {}) {
  ...
  void appendSwTraceEntry(entry).catch(() => {
  });
}
installReiSW(sw, {
