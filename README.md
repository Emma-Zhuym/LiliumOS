...
地图内另有独立的“我的位置”：可用免 Key 的 OpenStreetMap 点选、完整地图链接或经纬度，把家、学校和多个常去超市保存为本机地理围栏，并分别关联到每个角色的虚拟地图。角色仅在本地聊天中按需调用工具，收到`在家`、`在学校`、`在某家超市`或`在外面`等粗略语义，不会收到经纬度；iPhone PWA 仅支持前台读取。

### Finance 记账

多账户、多币种、层级分类、收入/支出/退款/转账、周期性交易、资产趋势和分类分析。SimpleFIN 可只读同步美国账户的余额与交易，同时保留本地昵称、图标、颜色、层级分类和备注；新交易采用渐进式整理，一级分类即可完成、模糊交易可以先放着，桌面角标只提示最近 7 天。稳定商户连续三次归入同一分类后会在本地自动学习，Amazon 等多品类商户保留人工确认与订单快捷入口。角色可主动查看近期交易、搜索账目、汇总支出和读取账户快照。
