...
- **手机即时聊天不再失联**：从哪台设备发送云端聊天，就先把回复目标登记回哪台设备；云端调用的成功、失败、token 和实际后端也会出现在该设备的「API 调用记录」里。
- **Worker 更新不再串包**：一键部署、自更新和手动修复统一取 LiliumOS 自己的 Worker；更新后必须重新读回版本，仍是旧包时不会再假报成功。
