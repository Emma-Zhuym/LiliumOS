...
- **每角色独立心跳**：可选约 30 分钟、1 / 2 / 4 小时醒来一次，实际时间带自然浮动；网页或 PWA 关闭后仍由 Cloudflare Worker 滚动续排。
- **角色自行决定是否开口**：醒来后可读取上下文和获准工具，也可安静跳过；热聊时让路，并受连续主动发送上限保护。
- **睡眠硬闸**：按角色时区优先遵守“日程 / 情绪”里的跨午夜睡眠区间，也识别当天日程中的睡眠、午睡和休眠；睡着时不调用模型、副 API 或工具。
- **热聊时可选行为**：心跳撞上正在聊天时，每个角色可选“自然融入当前话题”或“跳过本轮”；默认跳过以避免突然插话和额外调用。
- **可选省钱通道**：心跳可复用当前角色已保存的情绪副 API，不改变普通聊天、即时对话或普通排程的 API 路由。

协议、部署顺序与 Worker 更新要求见 [`docs/amsg2-heartbeat.md`](./docs/amsg2-heartbeat.md)。
