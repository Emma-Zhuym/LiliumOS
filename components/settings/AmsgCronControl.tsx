...
export default function AmsgCronControl({ refreshKey, disabled, onNeedsCredentials, onBusyChange, notify }: {
    ...
    return <section className="space-y-3 p-3" style={{ background: F.surfaceSunken, borderRadius: R.input, boxShadow: S.sunken, color: F.textSecondary }}>
        ...
        {known ? <button type="button" disabled={disabled || busy || checking} onClick={() => state!.enabled ? setConfirming(true) : void apply(true)} className="min-h-11 w-full text-xs font-semibold disabled:opacity-50" style={{ background: F.surface, border: `1px solid ${F.borderSoft}`, borderRadius: R.button, boxShadow: S.raisedSoft, color: F.textPrimary }}>{busy ? '正在处理…' : state!.enabled ? '暂停后台任务' : '恢复后台任务'}</button>
            : state?.code === 'CF_TOKEN_MISSING' ? <button type="button" disabled={disabled || busy} onClick={onNeedsCredentials} className="min-h-11 w-full text-xs font-semibold disabled:opacity-50" style={{ background: F.surface, border: `1px solid ${F.borderSoft}`, borderRadius: R.button, boxShadow: S.raisedSoft, color: F.textPrimary }}>为后端补充管理凭据</button>
