...
import { resolveCharacterApiConfig } from '../utils/characterApi'; // [EM: qixi-character-api]
...
const QIXI_FOOTER_NOTE = '首次生成会调用 4 次模型 API · 重看不会再次调用'; // [EM: qixi-api-call-copy]
...
export const SpecialMomentsApp: React.FC = () => {
    const { closeApp, openApp, characters, setActiveCharacterId, addToast, updateCharacter, apiConfig, apiPresets, userProfile } = useOS();
    ...
    const handleQixiReturnToChat = async ({ message, card, replaySnapshot }: QixiReturnPayload) => {
        ...
        try {
            ...
            if (!cardAlreadySaved) await DB.saveMessage(cardMessage);
            if (!messageAlreadySaved) await DB.saveMessage(privateMessage);
            if (targetChar) {
    ...
    const qixiApiConfig = resolveCharacterApiConfig(qixiChar, apiConfig, apiPresets).apiConfig; // [EM: qixi-character-api]
    if (showQixiSession && qixiChar) {
        return (
            <QixiDemoSession
                char={qixiChar}
                user={userProfile}
                apiConfig={qixiApiConfig}
