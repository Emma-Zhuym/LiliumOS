...
const StoryPagination: React.FC<{ page: number; pageCount: number; onChange: (page: number) => void; className?: string }> = ({ page, pageCount, onChange, className = '' }) => (
    <nav className={`${className} py-2 border-y border-slate-200 flex items-center justify-between`}>
        <button disabled={page === 0} onClick={() => onChange(Math.max(0, page - 1))} className='w-11 h-11 grid place-items-center disabled:opacity-20' style={{ borderRadius: R.pill, background: F.surface, boxShadow: S.raisedSoft, color: F.textSecondary }} aria-label='更早一页'><CaretLeft size={17} /></button>
        ...
        <button disabled={page >= pageCount - 1} onClick={() => onChange(Math.min(pageCount - 1, page + 1))} className='w-11 h-11 grid place-items-center disabled:opacity-20' style={{ borderRadius: R.pill, background: F.surface, boxShadow: S.raisedSoft, color: F.textSecondary }} aria-label='更新一页'><CaretRight size={17} /></button>
    </nav>
);
