...
const cleanupStyle = {
    '--cleanup-surface': F.surface, '--cleanup-text': F.textPrimary, '--cleanup-muted': F.textSecondary,
    '--cleanup-line': F.borderSoft, '--cleanup-danger': STATUS.danger.ink, '--cleanup-tint': STATUS.danger.tint,
    '--cleanup-shadow': S.raisedSoft, '--cleanup-inset': S.sunken, '--cleanup-radius': `${R.button}px`,
} as React.CSSProperties;
const cleanupCSS = `
.em-history-cleanup { color:var(--cleanup-text) }
.em-history-cleanup button { min-height:44px; padding:8px 12px; border-radius:var(--cleanup-radius); background:var(--cleanup-surface); color:var(--cleanup-text); border:1px solid var(--cleanup-line); box-shadow:var(--cleanup-shadow) }
.em-history-cleanup button:disabled { opacity:.4; cursor:default }
.em-history-cleanup input { min-height:44px; background:var(--cleanup-surface); color:var(--cleanup-text); border:0; border-radius:var(--cleanup-radius); box-shadow:var(--cleanup-inset) }
.em-history-cleanup [class*="text-slate-"] { color:var(--cleanup-muted) }
.em-history-cleanup [class*="text-violet-"] { color:var(--cleanup-text) }
.em-history-cleanup [class*="bg-slate-"], .em-history-cleanup [class*="bg-white"] { background:var(--cleanup-surface) }
.em-history-cleanup [class*="border-slate-"] { border-color:var(--cleanup-line) }
.em-history-cleanup [class*="bg-red-"], .em-history-cleanup [class*="border-red-"] { background:var(--cleanup-tint); border-color:var(--cleanup-line) }
.em-history-cleanup [class*="text-red-"] { color:var(--cleanup-danger) }
`;
function Modal({ children, footer, ...props }: React.ComponentProps<typeof ClayDialog>) {
    return <ClayDialog {...props} footer={footer && <div className="em-history-cleanup flex gap-3" style={cleanupStyle}>{footer}</div>}>
        <style>{cleanupCSS}</style><div className="em-history-cleanup" style={cleanupStyle}>{children}</div>
    </ClayDialog>;
}
