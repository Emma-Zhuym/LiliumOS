  ...
  return <div className="mt-5">
    ...
    <svg viewBox={`0 0 ${width} 184`} className="w-full block" aria-label={`${monthly ? '每月' : '每日'}支出柱状图`}>
      ...
      {series.map((day, index) => {
        ...
        const barPath = (amount: number) => {
          const h = amount / max * height;
          const x0 = x + (step - barWidth) / 2, x1 = x0 + barWidth, y = bottom - h;
          const r = Math.min(R.chartBar, barWidth / 4, h / 4);
          return `M ${x0} ${bottom} V ${y + r} Q ${x0} ${y} ${x0 + r} ${y} H ${x1 - r} Q ${x1} ${y} ${x1} ${y + r} V ${bottom} Z`;
        };
        return <g key={day.date} role="button" tabIndex={0}
          ...
          <path d={barPath(day.all)} fill={HUE.indigo.tint} />
          <path d={barPath(day.selected)} fill={day.date === selectedDate ? HUE.indigo.ink : HUE.indigo.main} />
