 ...
 * 恢复后按任务各自的到期、过期和循环规则处理；不保证所有旧任务都会补发。
 *
...
const readSchedules = (result: unknown): Array<{ cron: string }> | null => {
  const schedules = (result as { schedules?: unknown } | null)?.schedules;
  return Array.isArray(schedules) && schedules.every(item => item && typeof item.cron === 'string' && item.cron.trim()) ? schedules : null;
};
...
export async function handleCronTriggerRead(
  ...
  const schedules = readSchedules(current.result);
  if (!schedules) return { supported: false, ...fail('CF_ERROR', '定时触发回执不完整，无法确认状态。请重新读取。') };
  return { supported: true, enabled: schedules.length > 0 };
}
...
export async function handleCronTriggerWrite(
  ...
  if (!written.ok) {
    return {
      ...
      ...fail(
        'CF_ERROR',
        `${enabled ? '恢复' : '暂停'}未能确认（${written.detail}）。请重新读取定时触发状态。`,
      ),
    };
  }
  const confirmed = readSchedules(written.result);
  if (!confirmed || (enabled ? confirmed.length !== 1 || confirmed[0].cron !== AMSG_CRON_EXPRESSION : confirmed.length !== 0)) {
    return { ok: false, ...fail('CF_ERROR', '定时触发回执未能确认本次修改，请重新读取状态。') };
  }
