...
export function fridgeLots(lots: KitchenLot[]): KitchenLot[] {
  return lots.filter(lot => lot.quantity > 0 && lot.storageZone === 'fridge')
    .sort((a, b) => a.createdAt - b.createdAt || a.id.localeCompare(b.id));
}

export function fridgePage(lots: KitchenLot[], foods: KitchenFood[], requestedPage: number) {
  const available = fridgeLots(lots);
  const pageCount = Math.max(1, Math.ceil(available.length / FRIDGE_PAGE_SIZE));
  const page = Math.max(0, Math.min(Math.floor(requestedPage), pageCount - 1));
  const names = new Map(foods.map(food => [food.id, food.name]));
  const entries = available.slice(page * FRIDGE_PAGE_SIZE, (page + 1) * FRIDGE_PAGE_SIZE).map((lot, index) => {
    const name = names.get(lot.foodId) ?? '未命名食物';
    return {
      lot, name, model: kitchenModelFor(name),
      position: [(index % 3 - 1) * 0.36, [1.22, 0.76, 0.3][Math.floor(index / 3)], 0.12] as [number, number, number],
    };
  });
