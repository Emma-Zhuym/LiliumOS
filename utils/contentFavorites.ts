...
export type ContentFavoriteOwner =
    | { kind: 'user'; favoritedAt: number }
    | { kind: 'character'; charId: string; charName: string; favoritedAt: number };

export type ContentFavoriteOwnerSelector =
    | { kind: 'user' }
...
interface ContentFavoriteBase {
    ...
    favoritedAt: number;
    /** One card can belong to the user and one or more characters without duplication. */
    owners: ContentFavoriteOwner[];
}
...
let legacyGallerySyncStarted = false;
...
const ownerKey = (owner: ContentFavoriteOwner | ContentFavoriteOwnerSelector): string => (
    owner.kind === 'user' ? 'user' : `character:${owner.charId}`
);

const sanitizeOwner = (value: unknown): ContentFavoriteOwner | null => {
    if (!value || typeof value !== 'object') return null;
    const owner = value as Partial<ContentFavoriteOwner> & Record<string, unknown>;
    if (owner.kind === 'user') {
        return { kind: 'user', favoritedAt: normalizeTimestamp(owner.favoritedAt, Date.now()) };
    }
    if (owner.kind === 'character' && typeof owner.charId === 'string' && owner.charId) {
        return {
            kind: 'character',
            charId: owner.charId,
            charName: typeof owner.charName === 'string' && owner.charName ? owner.charName : '未知角色',
            favoritedAt: normalizeTimestamp(owner.favoritedAt, Date.now()),
        };
    }
    return null;
};

const mergeOwners = (...groups: ContentFavoriteOwner[][]): ContentFavoriteOwner[] => {
    const merged = new Map<string, ContentFavoriteOwner>();
    groups.flat().forEach(owner => {
        const key = ownerKey(owner);
        const previous = merged.get(key);
        if (!previous || owner.favoritedAt < previous.favoritedAt) merged.set(key, owner);
    });
...
const sanitizeFavorite = (value: unknown): ContentFavorite | null => {
    ...
    const base = {
        ...
        // Old upstream indexes did not record provenance; those were user-created.
        owners: mergeOwners(
            (Array.isArray(item.owners) ? item.owners : [])
                .map(sanitizeOwner)
                .filter((owner): owner is ContentFavoriteOwner => !!owner),
            Array.isArray(item.owners) ? [] : [{ kind: 'user' as const, favoritedAt: normalizeTimestamp(item.favoritedAt, now) }],
        ),
    };
 ...
 */
export const syncLegacyGalleryFavorites = async (): Promise<void> => {
    if (legacyGallerySyncStarted) return;
    legacyGallerySyncStarted = true;
    const [images, characters] = await Promise.all([
        DB.getGalleryImages().catch(() => []),
        DB.getAllCharacters().catch(() => []),
    ]);
    const favoriteImages = images.filter(image => image.favorited);
    if (!favoriteImages.length) return;

    const characterNames = new Map(characters.map(character => [character.id, character.name]));
    const legacyCharacterOwners = new Map<string, ContentFavoriteOwner[]>();
    await Promise.all([...new Set(favoriteImages.map(image => image.charId))].map(async charId => {
        const messages = await DB.getMessagesByCharId(charId, true).catch(() => []);
        messages.forEach(message => {
            const imageId = message.metadata?.kind === 'fav_photo' ? message.metadata?.imageId : undefined;
            if (typeof imageId !== 'string') return;
            const owners = legacyCharacterOwners.get(imageId) || [];
            owners.push({
                kind: 'character',
                charId,
                charName: characterNames.get(charId) || '未知角色',
                favoritedAt: normalizeTimestamp(message.timestamp, Date.now()),
            });
            legacyCharacterOwners.set(imageId, owners);
        });
    }));

    for (const image of favoriteImages) {
        const exactOwners: ContentFavoriteOwner[] = [
            ...(image.favoriteOrigins?.user ? [{ kind: 'user' as const, favoritedAt: image.timestamp }] : []),
            ...Object.entries(image.favoriteOrigins?.characters || {}).map(([charId, detail]) => ({
        ...
        const owners = exactOwners.length
            ? exactOwners
            : [{ kind: 'user' as const, favoritedAt: image.timestamp }, ...(legacyCharacterOwners.get(image.id) || [])];
        let persistedImage = image;
        if (!image.favoriteOrigins) {
            persistedImage = await DB.updateGalleryImageFavorite(image.id, true, { kind: 'user' });
            for (const owner of owners) {
                if (owner.kind !== 'character') continue;
                persistedImage = await DB.updateGalleryImageFavorite(image.id, true, {
                    kind: 'character',
                    charId: owner.charId,
                    charName: owner.charName,
                    favoritedAt: owner.favoritedAt,
                });
            }
        }
        const charName = characterNames.get(image.charId) || '未知角色';
        for (const owner of owners) await saveGalleryImageContentFavorite(persistedImage, charName, owner);
    }
};
...
/** Removes only one owner. Character-created favorites survive a user's unstar action. */
export const removeContentFavoriteOwnerById = async (
    id: string,
    ...
    if (owners.length === existing.owners.length) return false;
    if (owners.length) {
        await saveIndex(current.map(item => item.id === id ? { ...item, owners } : item));
    } else {
        await saveIndex(current.filter(item => item.id !== id));
        if (existing.kind === 'image') {
            await Promise.all(existing.references
...
/** User-facing unstar: clear the user origin in Gallery as well as the unified index. */
export const removeUserContentFavoriteById = async (id: string): Promise<boolean> => {
    const favorite = await getContentFavoriteById(id);
    if (!favorite) return false;
    if (favorite.kind === 'image') {
        await Promise.all(favorite.references
            .filter((reference): reference is Extract<ContentFavoriteReference, { source: 'gallery' }> => reference.source === 'gallery')
            .map(reference => DB.updateGalleryImageFavorite(reference.galleryImageId, false, { kind: 'user' }).catch(() => undefined)));
    }
    return removeContentFavoriteOwnerById(id, { kind: 'user' });
};
