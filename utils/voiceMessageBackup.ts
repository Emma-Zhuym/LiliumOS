/**
 * Voice favorites live in IndexedDB's
 ...
 * voice rows. The text index is always retained. Favorite audio passes through in full/media mode.
 */
export const shouldIncludeVoiceRelatedAssetInBackup = (value: unknown, includeFavorites = true): boolean => {
    ...
    if (asset.id === VOICE_FAVORITES_INDEX_ASSET_ID) return true; // [EM: text-voice-favorites]
    if (asset.id.startsWith(VOICE_FAVORITE_AUDIO_PREFIX)) return includeFavorites;
...
/** Only mutate the exported clone: text archives retain every text snapshot. */
export const markVoiceFavoriteAudioOmitted = (assets: unknown): void => {
    if (!Array.isArray(assets)) return;
    for (const asset of assets) {
        if (asset?.id !== VOICE_FAVORITES_INDEX_ASSET_ID) continue;
        const items = Array.isArray(asset.data) ? asset.data : asset.data?.items;
        if (!Array.isArray(items)) continue;
        for (const item of items) {
            if (item && typeof item === 'object' && item.audioState !== 'none') item.audioState = 'omitted';
        }
    }
};
...
/**
 * Mutates the export clone in place. Dedicated favorite audio rows and the legacy
 * per-chat favorite shape are externalized;
...
export async function externalizeVoiceMessageBlobs(
    ...
    for (let index = 0; index < assets.length; index++) {
        ...
        const backupDir = VOICE_BACKUP_DIR;
...
export async function restoreVoiceMessageBlobs(
    ...
    for (const raw of assets) {
        ...
        if (!isRestorableVoiceAsset(asset) || !asset.data) continue;
        // [EM-START: text-voice-favorites]
        const candidate = asset.data.blob;
        if (!candidate || typeof candidate !== 'object' || !(VOICE_BACKUP_MARKER in candidate)) continue;
        if (!isVoiceBackupBlobMarker(candidate)) {
            throw new Error('损坏的备份包：语音附件标记无效，已中止导入（数据未改动）。');
        }
        // [EM-END: text-voice-favorites]
