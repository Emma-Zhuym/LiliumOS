...
export const describeGithubUploadTransportFailure = (
    ...
    return `${prefix}。当前正在直连 GitHub 附件域名 uploads.github.com；它与 github.com、api.github.com 是不同线路。`
        + '能打开 GitHub、Token 测试通过或开着梯子，都不代表附件域名已被代理接管；若愿意使用应用内 Cloudflare 中转，'
        + '可到「GitHub 备份 → 通过 Cloudflare 中转备份」手动开启后重试。';
};
...
export const uploadBackup = async (
    ...
    let releaseId = 0;
    let tag = '';
    // [EM-START: github-publish-receipt-recovery]
    let publishAttempted = false;
    const acknowledgedAssets: GithubAsset[] = [];
    // [EM-END: github-publish-receipt-recovery]
    try {
        onProgress?.(2);
        const ts = Date.now();
        ...
        for (let i = 0; i < totalParts; i++) {
            ...
            acknowledgedAssets.push(result.asset); // [EM: github-publish-receipt-recovery]
        }
        ...
        acknowledgedAssets.push(manifestResult.asset); // [EM: github-publish-receipt-recovery]
        publishAttempted = true; // [EM: github-publish-receipt-recovery]
        ...
        // [EM-START: github-publish-receipt-recovery]
        // A lost publish response is not evidence that publishing failed. Once
        // PATCH has started, never delete a release that may already be usable.
        if (releaseId && publishAttempted) {
            try {
                const receipt = await ghRequest(
                    config,
                    `${API_HOST}/repos/${owner}/${repo}/releases/${releaseId}`,
                    'GET',
                    { headers: authHeaders(token) },
                ...
                if (receipt.status === 200) {
                    const release = await receipt.json();
                    if (release.draft === false) {
                        const assets = await listReleaseAssets(config, releaseId);
                        const complete = acknowledgedAssets.every(expected => assets.some(asset =>
                            asset.id === expected.id && asset.name === expected.name
                            && isUsableAsset(asset, expected.size)));
                        if (complete) {
                            onProgress?.(100);
                            return { ok: true, message: '上传成功（已重新核对 GitHub 发布状态和全部附件）' };
                        }
                    }
                }
            } catch { /* Preserve the backup when read-back is also unavailable. */ }
            return {
                ok: false,
                message: `附件已上传，但未能确认最终发布状态：${reason}；已保留 GitHub 备份，请刷新恢复列表或在 GitHub 检查后再决定是否重新上传。`,
            };
        }
        // [EM-END: github-publish-receipt-recovery]
        if (releaseId) {
    ...
    try {
        ...
        if (
            ...
            throw new Error(
                ...
                + '可到「GitHub 备份 → 通过 Cloudflare 中转备份」手动开启 Cloudflare 中转后重试；应用不会自动开启。',
            );
        }
