...
export const probeSwChannel = async (waitMs = 3_000): Promise<SwChannelProbeResult> => {
  const result: SwChannelProbeResult = { portAck: false, clientsAck: false, waitedMs: 0 };
  ...
  return await new Promise<SwChannelProbeResult>((resolve) => {
    ...
    const onClientsMessage = (event: MessageEvent) => {
      if (settled || event.data?.type !== 'sw-channel-probe-ack' || event.data?.nonce !== nonce) return;
    ...
    try {
      ...
      channel.port1.onmessage = (event: MessageEvent) => {
        if (settled || event.data?.type !== 'sw-channel-probe-port-ack' || event.data?.nonce !== nonce) return;
        result.portAck = true;
