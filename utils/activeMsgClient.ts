...
export const ActiveMsgClient = {
   ...
   *
   * 暂停期间任务保留；恢复后按各自的过期与循环规则处理，不保证全部补发。
  ...
  async setCronTriggerEnabled(enabled: boolean): Promise<{ ok: boolean; message: string; code?: string }> {
    ...
    if (status === 200 && body?.success === true) {
      if (body.data?.ok !== true || body.data?.enabled !== enabled) {
        return {
          ok: false,
          message: typeof body.data?.message === 'string' ? body.data.message : '后台回执未能确认本次修改，请重新读取状态。',
  ...
  async clearClientStateValue(namespace: string, key: string): Promise<void> {
    ...
    await putClientStateOrThrow(client, [{ namespace, key, value, updatedAt: stampStateUpdatedAt() }], '清理已取回的云端内容');
  },
