...
it.each([{success:true}, {success:true,data:{ok:false,enabled:false}}, {success:true,data:{ok:true,enabled:true}}])('pause does not report success for missing or contradictory worker receipt %j',async(body)=>{workerReplies(200,body);expect((await ActiveMsgClient.setCronTriggerEnabled(false)).ok).toBe(false);});
