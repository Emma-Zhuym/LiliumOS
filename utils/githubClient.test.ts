...
describe('GitHub 事务式上传', () => {
    ...
    it.each(['acknowledged', 'lost-response', 'read-unavailable', 'still-draft', 'missing-asset'])(
        '发布回执 %s 时只确认完整备份，无法确认也不误删', async (publishState) => {
        let assetId = 100;
        const uploadedNames: string[] = [];
        const uploadedAssets: { id: number; name: string; size: number; state: string }[] = [];
        class SuccessfulUploadXhr {
            ...
            send(body: Blob) {
                ...
                this.responseText = JSON.stringify(asset);
                this.onload?.();
            }
        }
        ...
        const fetchMock = vi.fn().mockImplementation((input: RequestInfo | URL, init?: RequestInit) => {
            ...
            if (url.endsWith('/releases/88') && init?.method === 'PATCH') {
                ...
                if (publishState !== 'acknowledged') throw new TypeError('publish response lost');
                return Promise.resolve(new Response(JSON.stringify({ id: 88, draft: false }), {
                    status: 200,
                    headers: { 'Content-Type': 'application/json' },
                }));
            }
            if (url.endsWith('/releases/88') && init?.method === 'GET') {
                if (publishState === 'read-unavailable') throw new TypeError('read unavailable');
                return Promise.resolve(new Response(JSON.stringify({ id: 88, draft: publishState === 'still-draft' })));
            }
            if (url.includes('/releases/88/assets?') && init?.method === 'GET') {
                return Promise.resolve(new Response(JSON.stringify(
                    publishState === 'missing-asset' ? uploadedAssets.slice(0, 1) : uploadedAssets,
                )));
            }
            throw new Error(`unexpected request: ${init?.method} ${url}`);
        });
        ...
        expect(result.ok).toBe(['acknowledged', 'lost-response'].includes(publishState));
        if (!result.ok) expect(result.message).toContain('已保留 GitHub 备份');
