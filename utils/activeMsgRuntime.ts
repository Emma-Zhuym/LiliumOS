...
const processInboxMessageWithPostProcessing = async (
  ...
  await applyAssistantPostProcessing(message.body || '', {
    ...
    api: {
      ...
      // push 也会复用后处理里的 [[SEND_PHOTO]]，因此这里需要带上全局生图配置；
      // 聊天 LLM 的角色预设仍由 Worker 决定，不在这里重新切换。
    ...
    instantRender: message.receivedWhileVisible === false
      || shouldRenderInstantly(message.metadata, message.receivedAt, Date.now()),
  });
