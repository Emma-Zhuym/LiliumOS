...
describe('content favorites reference index', () => {
    ...
    it('keeps user and character image ownership independent on one card', async () => {
        const image: GalleryImage = {
            id: 'favorite-owner-image',
            charId: CHAR_ID,
            url: 'data:image/png;base64,T1dORVJT',
            timestamp: 202,
        };
        await DB.saveGalleryImage(image);
        const userImage = await DB.updateGalleryImageFavorite(image.id, true, { kind: 'user' });
        await saveGalleryImageContentFavorite(userImage, 'Sully', { kind: 'user', favoritedAt: 203 });
        const sharedImage = await DB.updateGalleryImageFavorite(image.id, true, {
            kind: 'character',
            charId: CHAR_ID,
            charName: 'Sully',
            favoritedAt: 204,
        });
        const favorite = await saveGalleryImageContentFavorite(sharedImage, 'Sully', {
            kind: 'character',
            charId: CHAR_ID,
            charName: 'Sully',
            favoritedAt: 204,
        ...
        expect((await listContentFavorites())[0].owners).toHaveLength(2);
        await DB.updateGalleryImageFavorite(image.id, false, { kind: 'user' });
        await removeContentFavoriteOwnerById(favorite.id, { kind: 'user' });

        const remaining = (await listContentFavorites())[0];
        expect(remaining.owners).toEqual([expect.objectContaining({ kind: 'character', charId: CHAR_ID })]);
        expect(await DB.getGalleryImageById(image.id)).toMatchObject({
            favorited: true,
