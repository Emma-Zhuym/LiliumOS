...
describe('flushInboxToChat 落库时间戳（走真库）', () => {
  ...
  it('后台收到的消息跳过拟人打字延迟，前台实时收到的照旧慢放', async () => {
    const runFlush = async (charId: string, receivedAt: number, deliveredWhileAway = false) => {
      ...
      notePageBecameVisible(deliveredWhileAway ? receivedAt + 1_000 : receivedAt - 1_000);
      const t0 = Date.now();
      try {
        await flushInboxToChat();
        return Date.now() - t0;
    ...
    const backgroundMs = await runFlush('char-pace-background', Date.now(), true);
