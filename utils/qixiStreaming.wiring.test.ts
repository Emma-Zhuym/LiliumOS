...
describe('Qixi streaming request wiring', () => {
    ...
    it('keeps all Qixi generation calls on the zero automatic retry path', () => {
        expect(part1Source).toMatch(/safeFetchJson\([\s\S]*?\},\s*0,\s*QIXI_PART1_TIMEOUT_MS/);
        expect(part3Source).toMatch(/safeFetchJson\([\s\S]*?\},\s*0,\s*QIXI_PART3_TIMEOUT_MS/);
    });
