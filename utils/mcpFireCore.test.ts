...
describe('JSON-RPC 传输层（直连路径）', () => {
    ...
    it('工具中途要求输入时停止，不自动回答或重复执行工具', async () => {
        const sent = stubFetch(({ body }) => {
            if (body.method === 'initialize') return jsonResp({ jsonrpc: '2.0', id: body.id, result: {} });
            if (body.method === 'notifications/initialized') return new Response('', { status: 202 });
            return jsonResp({ jsonrpc: '2.0', id: body.id, result: { resultType: 'input_required' } });
        });
        const result = await callMcpToolCore(directTarget(directServer()), createMcpSessionState(), 'delete_note', {});
        expect(result).toMatchObject({ success: false, data: { resultType: 'input_required' } });
        expect(result.error).toContain('补充确认或输入');
        expect(sent.map(request => request.body.method)).toEqual(['initialize', 'notifications/initialized', 'tools/call']);
    });
