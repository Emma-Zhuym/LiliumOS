...
>
> 一个下面正文里没提到的坑：门牌输出的 `basedOn: "U0"` **是相对本次快照的编号**，跟提取那条一样需要还原。做法是提交时把每条的 id 一起带上、结果原样回传，落地时按 id 把标签重新对准当前条目（`remapBasedOnLabels`）。不做的话，结果晚回来那几分钟里门牌被别的路径动过，两条认知的来历就被悄悄接错了。
