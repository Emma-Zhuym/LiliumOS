...
- 角色详细查询：通过高层健康工具按需读今天/趋势，不直接读全部原始样本。
...
- [ ] 今天摘要、七日趋势、最近 workout 的角色按需工具。
- [ ] 去重、时区、过期数据和 HA 离线降级测试。

当前实现已按 Jamie Hill 配套集成公开的实体契约覆盖全部 27 项指标，并能把今日快照与最近
workout 注入角色健康摘要；明确健康查询会启用本地语义路由，首页只展示最常用的四项，避免
把 Health App 变成传感器列表。独立的七日趋势高层工具仍未完成。缓存新鲜度、HA 离线回退、
英美制单位换算和无关实体过滤已有回归测试；真实实体后缀、时区与重复数据仍需在首次手机同步后验收。
