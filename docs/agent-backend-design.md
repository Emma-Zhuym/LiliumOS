...
**做哪件事也由程序定**（`pickLifeKind`）：平时 聊天 48% / 网购 18% / 朋友圈 20% / 外卖 10% / 给阿萌买东西 4%，饭点（11:00–13:30、17:00–20:30）外卖提到 35%。让模型自己挑，它会一直挑同一种。

`gift`（给阿萌买东西）是唯一允许想到她的一种：网购还是点外卖（`via`）、要不要当惊喜（`surprise`）、附言（`note`）都由 TA 定。落地时记进 TA 手机的淘宝 / 外卖，同时进投喂站成为「来自 TA」的订单（`utils/shoppingFamily.ts` 的 `giftOrderFromLife`）；惊喜在送到前对她隐藏内容，偷看 TA 手机则能提前看到。
