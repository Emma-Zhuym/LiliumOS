...
单角色私聊里，工具完成后会在本轮角色回复下方留下可展开的「MCP 调用」卡片：
折叠状态只显示工具名与成功/失败；展开后可查看服务器名、参数、返回结果和耗时。
这份记录保存在对应回复的 metadata 中，不会作为聊天正文重新送给模型。Bearer Token、
API Key、Cookie、密码、自定义鉴权头等连接信息不进入记录；参数或结果里常见的敏感字段
会在落库前打码，过长内容只保留预览。
...
- **前台调用记录附着于最终回复 metadata**。`useChatAI.ts` 收集标准 function calling
  与正文兼容兜底两条路径的真实执行结果，经 `utils/mcpToolTraceRecord.ts` 打码、限长后写入
  `mcpToolTrace`；`apps/Chat.tsx` 只在同一轮最后一条气泡下渲染一次。不得改成独立系统消息，
  否则调用参数和结果可能进入下一轮模型历史。
