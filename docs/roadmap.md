...
- 已进入 3D 冰箱：圆角双门可各自点击开合，上冷冻下冷藏及底部双透明抽屉；点食材选中后可查看详情或移动到门内置物架，位置本地保存。门内三层置物架已支持上中下摆放，鸡蛋使用按剩余个数显示的 12 格蛋盒。下一步按 Emma 体验调整，再接旁边有门的 pantry；自由拖放、抽屉蔬果分配、余量变形、菜谱、Health 和角色查询尚未实现。
