...
> 状态：自定义直连与主 Worker 中转均已在线出图；角色画风预设、本地 API 调用记录、移动端结果归一化、失败重试和人物参考语义门控已接入。最后核对：2026-08-27。
...
2. 用户可接入支持图片输入的自定义接口，并仅在画面确实包含人物/可见人脸时用角色立绘维持身份特征；饭菜、物件、风景等纯场景不会上传立绘。
...
- `useCharacterReference` 缺省视为开启，但只对自定义接口且原始场景明确要求人物/可见人脸时生效。
- URL、Key、Model 经过与主 API 相同的边缘空白和不可见字符清理。
- 配置随 `apiConfig` 写入本地存储，并进入文字备份与完整备份；恢复时沿用现有 `updateApiConfig` 归一化入口。

## 角色参考图选择

启用立绘参考后，系统先读取未追加画风预设的原始场景描述：自拍、人像、可见角色等人物画面才进入参考图流程；`no people`、不露脸、背影、第一人称手部以及没有任何人物语义的饭菜/物件/房间/风景默认走纯文字生成。这样“电影写真”“细致五官”等画风词不会误触发身份图上传。

需要参考图时，按以下顺序选择第一张可用图片：
...
- 参考图是身份锚点，不是构图素材；提示词明确禁止把参考脸贴入、悬浮或放大到场景中，也要求人物肢体、相机位置、透视、倒影和与物体的空间关系符合现实。
- 不需要人物参考时，会在最终提示词末尾加入构图约束，禁止擅自添加角色、人脸、自拍、倒影人物或人体；原始场景明确要求的手部或不露脸人物仍可保留。
...
只有原始场景语义要求人物/可见人脸时才会调用此端点。场景描述前会加入身份保持说明，要求尽量维持脸型、眼睛、五官比例、发际线、发色和辨识特征，同时不复制参考图的姿势、表情、服装、构图与背景，并约束人物与镜头的物理关系。
...
- 自定义通道会把文字提示发送给用户填写的服务；开启角色参考且场景明确要求人物/可见人脸时，才会上传选中的角色立绘。
...
- 人物参考语义门控覆盖：自拍/人物正向识别、饭菜/物件默认不上传、背影/不露脸/POV 手部排除、画风人像词不得误触发、无人物提示追加禁止擅自加脸约束。
- 身份提示覆盖物理一致性：不粘贴/悬浮参考脸，不擅自改成自拍，保持肢体、镜头、透视、倒影与物体关系合理。
- 生图、画风预设、API 配置归一化、照片后处理与聊天提示词 6 个定向测试文件 69 passed；生产构建、`git diff --check` 与 `check-em-patches.sh` 79/79 通过。
